...
界面采用 **iOS 原生设计规范**（分组列表、大标题、胶囊控件、系统色板）并叠加 **Liquid Glass** 材质语言
（半透明玻璃底、边缘高光描边、柔和浮起阴影）。
...
| 作息设置 | 自选每天节数（上午 / 下午 / 晚间分别设置）、每节时长与课间休息，一键生成时间表；并支持逐节微调开始时间、结束时间与持续时间（三者联动） |
| 外观 | iOS 原生风格 + Liquid Glass 材质；浅色 / 深色 / 跟随系统，6 种系统强调色 |
