...
| 上课提醒 | 课前按设置的提前时间（5 / 10 / 15 / 30 分钟）弹出本地通知；**相邻的同一门课只提醒一次**；清理后台、划掉任务后仍然有效 |
...
提醒不使用常驻服务或轮询，而是把每个上课时刻注册为**系统闹钟**（`AlarmManager`），
到点由系统直接唤醒应用的广播接收器投递通知。因此：

| 场景 | 是否仍能收到提醒 |
| --- | --- |
| 应用被切到后台 | 能 |
| 清理后台 / 从最近任务里划掉 | 能（闹钟由系统持有，与进程无关） |
| 手机重启 | 能（开机后自动重排全学期的闹钟） |
| 手动修改系统时间或时区 | 能（重新排布，避免时刻漂移） |
| 两周以上没打开过应用 | 能（一次性预排未来两周，并有每日自动重排滚动补充） |
| 在系统设置里「强行停止」过应用 | 需重新打开一次应用（此时系统会撤销全部闹钟，属系统行为） |

设置页会在缺少授权时给出对应入口：

- **通知权限**（Android 13 起需用户授予）——缺失时闹钟照常触发，但看不到提示
- **精确闹钟**——未授权时退化为非精确闹钟，可能晚几分钟，但仍会提醒
