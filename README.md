...
界面遵循 vivo OriginOS 的设计语言：大圆角卡片、柔和阴影、渐变强调色、悬浮胶囊底部导航。

应用**不申请任何网络权限**，所有数据存放在应用私有目录的 SQLite 数据库中。
...
| 学期设置 | 开学日期、总周数、每日节数、每节课的上下课时间、当前周手动指定 |
| 外观 | 浅色 / 深色 / 跟随系统，6 种强调色，深色模式完整适配 |
...
└── ui/
    ...
    ├── theme/                          OriginOS 设计系统（色板 / 排版 / 形状）
    ├── components/                     卡片、按钮、分段控件、课表网格
    └── screens/                        课表页 / 今日页 / 导入页 / 设置页 / 课程编辑抽屉
```
